...
def get_observation_data_reformed(observation, t):
    ...
    return [t * STEP_IN_MINUTES / MINUTES_PER_HOUR, pH] + [
        eval(f"observation.{var}.y[t]", {"observation": observation, "t": t})
        for var in vars
    ]
...
class PenSimEnvGym(PenSimEnv, smplEnvBase):
    def __init__(
        ...
        self.init_observation_from_dataset = np.array(
            [0.2, 6.5, 300.0, 0.0, 10.828493, 0.0001, 150.0, 62500.0, 14.75],
            dtype=self.np_dtype,
        )  # init observation generated from dataset.
    ...
    def step(self, action, normalize=None):
        ...
        try:
            ...
            values_dict = self.recipe_combo.get_values_dict_at(
                self.step_count * STEP_IN_MINUTES
            )
            # served as a batch buffer below
            pensimpy_observation, x, yield_per_run, done = super().step(
                self.step_count,
                self.x,
                action[1],
                action[2],
...
class PeniControlData:
    ...
    def __init__(
        self,
        load_just_a_file="",
        dataset_folder="examples/example_batches",
        delimiter=",",
        observation_dim=9,
        action_dim=6,
        normalize=True,
    ...
    def load_file_list_to_dict(self, file_list, shuffle=True):
        ...
        if self.normalize:
            dataset["observations"], _, _ = normalize_spaces(
                dataset["observations"], self.max_observations, self.min_observations
            )
            dataset["next_observations"], _, _ = normalize_spaces(
                dataset["next_observations"],
                self.max_observations,
                self.min_observations,
            )
            dataset["actions"], _, _ = normalize_spaces(
                dataset["actions"], self.max_actions, self.min_actions
            )  # passed in a normalized version.
        # self.action_space = spaces.Box(low=-1, high=1, shape=(self.action_dim,))
        return dataset
