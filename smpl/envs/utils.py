...
from gym import (
    spaces,
    Env,
)  # to create an openai-gym environment https://gym.openai.com/
...
class TorchDatasetFromD4RL(Dataset):
    def __init__(self, dataset_d4rl) -> None:
        ...
        """
        self.dataset = d3rlpy.dataset.MDPDataset(
            dataset_d4rl["observations"],
            dataset_d4rl["actions"],
            dataset_d4rl["rewards"],
            dataset_d4rl["terminals"],
        )
...
class smplEnvBase(Env):
    def __init__(
        ...
        if self.normalize:
            self.observation_space = spaces.Box(
                low=-1, high=1, shape=(self.observation_dim,)
            )
            self.action_space = spaces.Box(low=-1, high=1, shape=(self.action_dim,))
        else:
            self.observation_space = spaces.Box(
                low=self.min_observations,
                high=self.max_observations,
                shape=(self.observation_dim,),
            )
            self.action_space = spaces.Box(
                low=self.min_actions, high=self.max_actions, shape=(self.action_dim,)
            )
    ...
    def action_beyond_box(self, action):
        ...
        return (
            ...
            or np.any(np.isnan(action))
            or np.any(np.isinf(action))
        )
    ...
    def reward_function_standard(
        ...
        Returns:
            [float]: reward.
        """
        if self.observation_beyond_box(current_observation) or self.action_beyond_box(
            action
        ):
        ...
        reward = max(
            self.error_reward, reward
        )  # reward cannot be smaller than the error_reward
        if self.debug_mode:
            print("reward:", reward)
        return reward

    def done_calculator_standard(
        self, current_observation, step_count, reward, done=None, done_info=None
    ):
    ...
    def observation_done_and_reward_calculator(
        self,
        current_observation,
        action,
        normalize=None,
        ...
        if (
            done_info["error_occurred"] is True
        ):  # error occured. Set reward to self.error_reward.
    ...
    def step(self, action, normalize=None):
        ...
        observation, reward, done, done_info = (
            self.observation_done_and_reward_calculator(
                observation,
                action,
                normalize=normalize,
                step_reward=reward,
    ...
    def evalute_algorithms(
        ...
        actions_list = [
            [] for _ in range(len(algorithms))
        ]  # actions_list[i][j][t][k] is algorithm_i_game_j_action_t_element_k
        rewards_list = [
            [] for _ in range(len(algorithms))
            ...
            if to_plt:
                ...
                for n_a in range(self.action_dim):
                    ...
                    for n_algo in range(len(algorithms)):
                        ...
                        _, algo_name, _ = algorithms[n_algo]
                        plt.plot(
                            np.array(actions_list[n_algo][-1])[:, n_a],
                            label=algo_name,
                            alpha=alpha,
                        )
                    plt.xticks(np.arange(1, self.max_steps + 2, 1))
                    ...
                    if plot_dir is not None:
                        path_name = os.path.join(
                            plot_dir, f"{n_epi}_action_{a_name}.png"
                        )
                        plt.savefig(path_name)
                    plt.close()
        ...
        for n_algo in range(len(algo_names)):
            ...
            for n_epi in range(num_episodes):
                if rewards_list_curr_algo[n_epi][-1] == self.error_reward:
                    ...
                    rewards_mean_over_episodes.append(
                        np.mean(rewards_list_curr_algo[n_epi])
                    )
            ...
            result_dict[algo_name + "_on_episodes_reward_mean"] = (
                on_episodes_reward_mean
            )
    ...
    def generate_dataset_with_algorithm(
        self,
        algorithm,
        normalize=None,
        num_episodes=1,
        ...
        for n_epi in tqdm(range(num_episodes)):
            ...
            while not final_done:
                ...
                tmp_o = o
                if normalize:
                    tmp_o, _, _ = normalize_spaces(
                        tmp_o, self.max_observations, self.min_observations
                    )
                curr_time = time.time()
